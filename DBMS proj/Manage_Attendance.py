...
class Class_Attendance:
    def __init__(self, root, ls, frame_old):
        ...
        # Labeling the title
        title = Label(self.frame, text='ATTENDENCE REGISTER', font=('Algerian', 25, 'bold'),
        ...
        lbl_roll = Label(Roll_Frame, text="ENTER THE ROLL NO.", bg="LightBlue", fg="RED",
        ...
        # Button
        CheckBtn = Button(Roll_Frame, text="FIND INFO", bg='black', fg='white', width=10, command=self.Attendance)
        CheckBtn.grid(row=3, column=0, pady=20, padx=150, sticky="w")
        # -----------

    def Attendance(self):
        conn = b=mysql.connector.connect(host="localhost",user="root",passwd='79sumathi',database="tv")
        cur = conn.cursor()
        ...
        if len(rows) != 0:
            ...
            if self.flag2 == 0:
                if self.flag1 == 0:
                    self.Attendence = Label(self.Manage_Frame,text="NO STUDENT EXISTS", anchor=CENTER, bg='cornsilk', font=("times new roman", 30))
                    ...
                    self.Attendence = Label(self.Manage_Frame, text="NO STUDENT DATA FOUND", anchor=CENTER,
