...
class Class_Fees:
    def __init__(self, root, ls, frame_old, roll):
        ...
        # Labeling the title
        title = Label(self.frame, text='FEE CHECKER', font=('Algerian', 25, 'bold'),
        ...
        # Text Variable.
        self.Txt_paid = Label(Paid_Frame, anchor=CENTER, text="PAID FEES", bg='lightblue',fg='red', font=("times new roman", 18))
        self.Txt_paid.place(x=0, y=ls[1]*50//714, width=ls[0]*300//1336-15)

        # Text Variable.
        self.Txt_rem = Label(Rem_Frame,anchor=CENTER, text="REMAINING FEES", bg='lightblue',fg='red', font=("times new roman", 18))
        ...
        if int(rows[0][1]) == 0:
            ...
            if int(rows[0][2]) == 0:
                self.Txt_fees.config(text="FULL FEES PAID")
            else:
