...
class Student_Menu:
    def __init__(self, root, ls, frame_old, frame_run, roll):
        ...
        # Labeling the page.
        self.title = Label(self.frame, text='TVD MOTORS', font=('Algerian', 25, 'bold'), bg="#163148", fg="#ffffff").pack(
            side=TOP)
        ...
        self.title1 = Label(self.frame2, text='CUSTOMER PORTAL', font=('Algerian', 22, 'bold'), bg='#ffffff').pack(
            side=TOP)
        ...
        self.ID_card = Button(self.frame2, text='CREATE ID CARD', bd=0, bg='#fbf8e6', image=self.photo_ID_Card, compound=TOP, command=self.IDGenerator) \
            .place(width=ls[0] // 9+10, height=ls[1]//6+10, x=ls[0]//9, y=ls[1] // 6)

        self.attendance = Button(self.frame2, text='ATTENDENCE PERCENTAGE', bd=0, bg='#fbf8e6', image=self.photo_attendance,
                              compound=TOP, command=self.AttendanceGenerator) \
            .place(width=ls[0] // 9+10, height=ls[1] // 6+10, x=ls[0] // 9*3, y=ls[1] // 6)

        self.marksheet = Button(self.frame2, text='REPORT CARD', bd=0, bg='#fbf8e6', image=self.photo_marksheet,
        ...
        self.Exit_btn = Button(self.frame2, text='LOG OUT', bd=0, bg='#fbf8e6', image=self.photo_exit,
