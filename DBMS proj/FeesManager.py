...
class ClassFees:
    def __init__(self, root, ls, frame_old):
        ...
        lbl_roll = Label(Roll_Frame, text="ENTER ROLL.NO", bg="LightBlue", fg="RED",
        ...
        # Button
        CheckBtn = Button(Roll_Frame, text="Check", bg='black', fg='white', width=10, command=self.Fees)
        ...
        if len(rows) != 0:
            if self.flag1 == 0:
                if self.flag2 == 0:
                    ...
                    # Labeling And Text.
                    self.lbl_paid = Label(self.Frame_paid,anchor=CENTER, text="FEES PAID",font=("times new roman", 16), bg='cornsilk', fg='blue')
                    self.lbl_paid.place(x=0,y=0,width=(self.ls[0] * 800 // 1336)//2-5, height=75)

                    # Labeling And Text.
                    self.lbl_paid = Label(self.Frame_rem, anchor=CENTER, text="FEES REMAINING",font=("times new roman", 16), bg='cornsilk', fg='blue')
                    self.lbl_paid.place(x=0, y=0, width=(self.ls[0] * 800 // 1336) // 2 - 5, height=75)
